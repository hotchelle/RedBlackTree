...
// Also includes tracing.  See 3318 notes.  BPW
...
//This function doesn't return anything and takes in a char *, which is the input string that contains the nodes of the red-black tree
//The purpose of this function is to deserialize the input string and build a red-black tree with it.
void STdeserialize(char *str)
{
  ...
  for (i = 0; i < size; i++)
  {
    ...
    if (str[i] == '-') // checking for optional signs
    {
      sign = -1;
      i++;
    }
    ...
    if (isdigit(str[i])) //check if digit
    {
      while (isdigit(str[i])) //skip and store all digits
      {
        dummy[dummy_track++] = str[i++];
      }
      x->item = t = atoi(dummy) * sign; // get the values stored
    }

    if (isalpha(str[i])) // check if it's r or b
    {
      if (str[i] == 'b')
      {
        x->red = 0;
        ...
        for (grabage = 0; grabage < sizeof(dummy); grabage++)
        {
          dummy[grabage] = '\0';
        }
      }
      if (str[i] == 'r')
      {
        ...
        if (str[i + 1] == '.' && str[i + 2] == '.') // special case for . to set the children to be NULL
        {
          x->l = x->r = NULL;
        }
        else if (str[i + 1] == '.')
...
//This function doesn't return anything and takes a head root and a char * to update the output string with values.
//The purpose of this function is to serilaize the red-black tree by pre-order traversal.
void STserialize(link root,char * output)
{
  if (root == NULL)
  {
    return;
  ...
  {
    if (root->item != NULLitem) //visiting node
    {
      if (root->red == 1)
      {
        sprintf(output, "%s%d%c", output,root->item,'r');
      }
      else if(root->red == 0)
      {
        sprintf(output, "%s%d%c", output,root->item,'b');
      }
    }
    else if(root->item == NULLitem)
    {
      sprintf(output, "%s.", output);
    }

    STserialize(root->l,output); // going to visit left child
    STserialize(root->r,output); // going to visit right child
  }
}

//The purpose of this function is to get the head of the root of the red-black tree
//This function takes no parameters and returns a link type which is the head of the red-black tree
link getHead()
{
  return head;
}

//This function doesn't return anything and takes the head node of the red-black tree
//The purpose of this function is to free allocated memory for each node
void freeMemory(link root)
{
    if(root == NULL)
    {
        free(root);
